...
Usage examples:
  ...
  python main.py --rate-limit 50     # limit 50 packets/s per source
  python main.py --watch             # hot-reload rules on file change
"""
...
SIMULATION_PACKETS = [
    ...
    Packet.tcp("10.0.0.5",      "192.168.1.10",  4444, 22,   flags="SYN"),  # SSH from LAN
    Packet.udp("192.168.1.10",  "8.8.8.8",       1234, 443),
    # Stateful test: SYN then ACK then data on same connection
    Packet.tcp("192.168.1.10",  "93.184.216.34",  2000, 80,  flags="SYN"),
    Packet.tcp("192.168.1.10",  "93.184.216.34",  2000, 80,  flags="ACK"),
    Packet.tcp("192.168.1.10",  "93.184.216.34",  2000, 80,  flags="ACK", size=512),
]
...
def run_simulation(fw: Firewall, fwlog: FirewallLogger):
    ...
    print("=" * 75)
    print(f"  {'#':<4} {'ACTION':<7} {'RULE':<30} {'PACKET'}")
    print("-" * 75)

    for i, pkt in enumerate(SIMULATION_PACKETS, 1):
        action, rule = fw.process(pkt)
        fwlog.log_event(action.value, pkt, rule)
        symbol = {"ALLOW": "[+]", "DENY": "[X]", "LOG": "[*]"}.get(action.value, "[?]")
        rule_str = rule.comment if rule and rule.comment else ("default" if not rule else "---")
        print(f"  {i:<4} {symbol} {action.value:<6} {rule_str:<30} {pkt}")
    ...
    # Show top talkers
    talkers = fw.state_table.top_talkers(5)
    if talkers:
        print("\n  Top Talkers:")
        print(f"    {'SRC':<22} {'DST':<22} {'BYTES':<10} {'PKTS':<6} STATE")
        print("    " + "-" * 70)
        for t in talkers:
            print(
                f"    {t['src']:<22} {t['dst']:<22} "
                f"{t['bytes']:<10} {t['packets']:<6} {t['state']}"
            )

    # Show rule hit counts
    print("\n  Rule Hit Counts:")
    stats = fw.rule_engine.rule_stats()
    active_rules = [s for s in stats if s["hit_count"] > 0]
    if active_rules:
        for s in active_rules:
            print(f"    [{s['hit_count']}x] {s['rule']}")
    else:
        print("    (no rules were hit)")
    print()
...
def run_live(fw: Firewall, fwlog: FirewallLogger, interface: str):
    ...
    while True:
        ...
        print(
            ...
            f"deny={s.packets_denied}  rate_limited={s.packets_rate_limited}  "
            f"conns={fw.state_table.count()}",
            end="",
            flush=True,
        )
...
def main():
    ...
    parser.add_argument("--rate-limit", type=int, default=0,  metavar="N",
                        help="Max packets/sec per source IP (0=disabled)")
    parser.add_argument("--watch",      action="store_true",
                        help="Watch rules file for changes and auto-reload")
    args = parser.parse_args()

    setup_logging(args.verbose)

    default_policy = Action.ALLOW if args.policy == "allow" else Action.DENY
    fw = Firewall(default_policy=default_policy, rate_limit=args.rate_limit)
    ...
    with FirewallLogger(log_file=args.log_file) as fwlog:
        if args.simulate:
            fw.start()
